...
6. This line shows how to call for a js hook. The equivalent of this line in regular jQuery looks like this: `$('[data-jshook^="module__function "], [data-jshook*=" module__function "], [data-jshook$=" module__function"], [data-jshook="module__function"]')`. See how much smaller it makes it? The reason for the selector being so long is to ensure jQuery treats the hooks similarly to classes (as in spaces are used to seperate multiple hooks). Just using `[data-jshook*="module__function"]` target things means `$('[data-jshook*="moduleName__functionName"])` could also target `'<div data-jshook="moduleName__functionName--modifier"></div>`.
